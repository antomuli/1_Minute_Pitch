...
def index():
    ...
    search  = 'product'
    promotion_pitches = Pitch.query.filter_by(category = search).all()


    return render_template('index.html',title = title, promotion = promotion_pitches)
...
def new_pitch():
    ...
    if pitch_form.validate_on_submit():
        ...
        pitch_content = pitch_form.pitch_content.data
        category = pitch_form.category.data

        # Updated pitch instanceh
        new_pitch = Pitch(pitch_title=pitch_title,pitch_content=pitch_content,category=category,user=current_user,)
